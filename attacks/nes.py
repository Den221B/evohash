"""Clean NES attack in float32 [0, 1] image space."""
...
def run_attack(
    ...
    max_iters = resolve_max_iters(cfg, budget=budget, queries_per_iter=2 * n_samples + 1)

    rng = np.random.default_rng(int(cfg.get("seed", seed)))
    x = to_float32(x_source).astype(np.float32).copy()
    best_dist = float(oracle.query(x))
    best_x = x.copy()
    history = [best_dist]
    ...
    for it in range(1, max_iters + 1):
        ...
        for _ in range(n_samples):
            if not oracle.budget_ok():
                break
            u = _gaussian_direction(rng, x.shape)
            f_pos = float(oracle.query(np.clip(x + sigma * u, 0.0, 1.0)))
            if not oracle.budget_ok():
                break
            f_neg = float(oracle.query(np.clip(x - sigma * u, 0.0, 1.0)))
            grad += (f_pos - f_neg) * u
            used += 1
        ...
        grad /= 2.0 * sigma * float(used)
        x = np.clip(x - lr * grad, 0.0, 1.0)
        current_dist = float(oracle.query(x))
