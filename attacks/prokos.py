"""Clean Prokos attack in float32 [0, 1] image space."""
...
def run_attack(
    ...
    for it in range(1, max_iters + 1):
        ...
        if antithetic:
            for _ in range(n_samples):
                if not oracle.budget_ok():
                    break
                p = sample_dir(rng, x.shape)
                f_pos = float(oracle.query(np.clip(x + sigma * p, 0.0, 1.0)))
                if not oracle.budget_ok():
                    break
                f_neg = float(oracle.query(np.clip(x - sigma * p, 0.0, 1.0)))
                g += (f_pos - f_neg) * p
                used += 1
        else:
        ...
        g_unit = g / norm
        delta = momentum * delta + (1.0 - momentum) * g_unit
        x = np.clip(x - lr * delta, 0.0, 1.0)
        current_dist = float(oracle.query(x))
